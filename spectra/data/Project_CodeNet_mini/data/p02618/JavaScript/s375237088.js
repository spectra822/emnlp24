...
function Main(){
	...
	var output = new Array(D);
	var lastContest = new Array(26).fill(0);
	for(var i = 1; i <= D; i++){
		...
		lastContest[maxIndex] = i;
		for(var j = 0; j < 26; j++){
			max -= clist[j] * (i - lastContest[j]);
		}
		if(i == 1){
			//output[i - 1] = max;
		}else{
			//output[i - 1] = max + output[i - 2];
		}
		myerr(max);
		output[i - 1] = maxIndex + 1;
		//output[i] = Math.ceil(Math.random() * 26);
	}
