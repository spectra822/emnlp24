...
from utils import RichLog, strip_noncode, remove_non_comments
from utils import comment_remover
from models import get_model_from_name, ModelException
from modules import compose_code_to_test, simple_compose_code_to_desc, simple_compose_code_to_invariant, simple_compose_desc_to_code, simple_compose_invariant_to_code
...
def select_test_case(test_cases, code, lang="c", num_to_select=1):
    ...
    for i, (sample_input, expected_output) in enumerate(test_cases):
        result = cnv.verify_with_custom_test(code, lang, sample_input, expected_output, eof=True, valgrind=True, coverage=True)
...
def reverse_gen_desc(desc, lang, model, input_data, ground_truth):
    ...
    desc_to_code_prompt = simple_compose_desc_to_code(desc, lang, input_data, ground_truth)
...
def reverse_gen_invariant(invariant, lang, model, input_data, ground_truth):
    ...
    invariant_to_code_prompt = simple_compose_invariant_to_code(invariant, lang, input_data, ground_truth)
...
def generate_desc(code, lang, model, input_data, ground_truth, self_consistency=False, num_candidates=6):
    ...
    desc_prompt = simple_compose_code_to_desc(code, lang, input_data, ground_truth)
    ...
    if self_consistency:
        ...
        descriptions = set(list(descriptions))
        similarities, selected_desc = [], None
        for i, desc in enumerate(descriptions):
            desc = strip_noncode(desc, retain_markdown=False)
            desc_minus_code = remove_non_comments(desc)
            RichLog.info(f"Attempt {i+1} of {len(descriptions)}")
            code_guess = reverse_gen_desc(desc_minus_code, lang, model, None, None)
...
def generate_invariant(code, lang, model, input_data, ground_truth, self_consistency=False, num_candidates=6):
    ...
    cnv = CodeNetVerifier()
    code = comment_remover(code)
    invariant_prompt = simple_compose_code_to_invariant(code, lang, input_data, ground_truth)
    ...
    if self_consistency:
        ...
        invariants = set(list(invariants))
        similarities, selected_invariant = [], None
        for i, invariant in enumerate(invariants):
            invariant = strip_noncode(invariant, retain_markdown=False)
            invariant_minus_code = remove_non_comments(invariant)
            RichLog.info(f"Attempt {i+1} of {len(invariants)}")
            code_guess = reverse_gen_invariant(invariant_minus_code, lang, model, None, None)
...
if __name__ == "__main__":
    ...
    for data in test_dataset.iterator():
        ...
        try:
            ...
            if args.src_lang == "javascript":
                original_code = jsbeautifier.beautify(original_code)
            ...
            if args.invariant:
                ...
                invariant_index, invariant = generate_invariant(original_code, args.src_lang, model, input_data, ground_truth, self_consistency=args.sc, num_candidates=3)
            ...
            if args.desc:
                ...
                desc_index, desc = generate_desc(original_code, args.src_lang, model, input_data, ground_truth, self_consistency=args.sc, num_candidates=3)
                if desc is None:
